...
class AC_Device():
    ...
    def get_status(self):
        response = {"type":"status", "temperature":self._get_temperature(), "switch_status":self._get_switch_status() ,"device_id": self._device_id, "room_type": self._room_type, "device_type":self._device_type }
        topic_name = "home"
        publish.single(topic=topic_name, payload=json.dumps(response), hostname=HOST)
